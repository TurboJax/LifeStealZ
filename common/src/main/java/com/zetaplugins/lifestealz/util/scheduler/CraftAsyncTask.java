...
class CraftAsyncTask extends CraftTask {
    ...
    CraftAsyncTask(final Map<Integer, CraftTask> runners, final Consumer<BukkitTask> task, final int id, final long delay) {
        super(task, id, delay);
    ...
    public void run() {
        ...
        thread.setName(nameBefore + " - LifeStealZ");
        try { synchronized (this.workers) { // Paper end - name threads according to running plugin
            ...
            try {
                ...
                thrown = t;
                LifeStealZ.LOGGER.warn("Plugin LifeStealZ generated an exception while executing task {}", this.getTaskId(), thrown);
            } finally {
                ...
                synchronized (this.workers) {
                    try {
                        ...
                        if (!removed) {
                            throw new IllegalStateException(String.format("Unable to remove worker %s on task %s for LifeStealZ", thread.getName(), this.getTaskId()), thrown); // We don't want to lose the original exception, if any
                        }
                    } finally {
