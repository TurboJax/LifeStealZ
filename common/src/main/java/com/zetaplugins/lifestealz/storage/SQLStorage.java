...
public abstract class SQLStorage extends Storage {
        ...
        try (Connection connection = getConnection()) {
            ...
            try (Statement statement = connection.createStatement()) {
                String sql = """
                        ...
                        uuid CHAR(36) PRIMARY KEY,\s
                        name VARCHAR(64) NOT NULL,\s
                        maxhp FLOAT NOT NULL DEFAULT 20.0,\s
                        hasbeenRevived SMALLINT NOT NULL DEFAULT 0,\s
                        craftedHearts SMALLINT UNSIGNED NOT NULL DEFAULT 0,\s
                        craftedRevives SMALLINT UNSIGNED NOT NULL DEFAULT 0,\s
                        killedOtherPlayers MEDIUMINT UNSIGNED NOT NULL DEFAULT 0,\s
                        firstJoin BIGINT UNSIGNED NOT NULL);
                        """;
                statement.executeUpdate(sql);

                migrateDatabase();
            } catch (SQLException e) {
                LifeStealZ.LOGGER.error("Failed to initialize SQL database:", e);
            }
        } catch (SQLException e) {
            LifeStealZ.LOGGER.error("Failed to initialize SQL database:", e);
        }
        ...
        try (Connection connection = getConnection()) {
            ...
            try (PreparedStatement statement = connection.prepareStatement(sql)) {
                ...
                try (ResultSet resultSet = statement.executeQuery()) {
                    ...
                    if (!resultSet.next()) {
                        OfflinePlayer player = getPlugin().getServer().getPlayer(uuid);
                    ...
                    LifeStealZ.LOGGER.error("Failed to load player data from SQL database:", e);
                    return null;
                }
            } catch (SQLException e) {
                LifeStealZ.LOGGER.error("Failed to load player data from SQL database:", e);
    ...
    public void save(PlayerData playerData) {
        ...
        try (Connection connection = getConnection()) {
            ...
            LifeStealZ.LOGGER.error("Failed to save player data:", e);
        }
    }
    ...
    private boolean checkIfEntryExists(Connection connection, String uuid) {
        ...
        try (PreparedStatement selectStmt = connection.prepareStatement(selectQuery);) {
            ...
            LifeStealZ.LOGGER.error("Failed to check if player entry exists:", e);
            return false;
        }
    }
    ...
    private boolean insertPlayerData(Connection connection, PlayerData playerData) {
        final String insertQuery = "INSERT INTO hearts (uuid, name, maxhp, hasbeenRevived, craftedHearts, craftedRevives, killedOtherPlayers, firstJoin) VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        ...
        try (PreparedStatement insertStmt = connection.prepareStatement(insertQuery)) {
            ...
            LifeStealZ.LOGGER.error("Failed to insert player data:", e);
            return false;
        }
    }
    ...
    private boolean updatePlayerData(Connection connection, PlayerData playerData) {
        ...
        for (String field : playerData.getModifiedFields()) {
            ...
            switch (field) {
                case "maxhp" -> params.add(playerData.getMaxHealth());
                case "hasbeenRevived" -> params.add(playerData.getHasBeenRevived());
                case "craftedHearts" -> params.add(playerData.getCraftedHearts());
                case "craftedRevives" -> params.add(playerData.getCraftedRevives());
                case "killedOtherPlayers" -> params.add(playerData.getKilledOtherPlayers());
                case "firstJoin" -> params.add(playerData.getFirstJoin());
            }
        }
        ...
        try (PreparedStatement updateStmt = connection.prepareStatement(updateQuery.toString())) {
            ...
            LifeStealZ.LOGGER.error("Failed to update player data:", e);
            return false;
        }
    }
    ...
    public List<UUID> getEliminatedPlayers() {
        ...
        int minHearts = getPlugin().getConfig().getMinHearts();
        ...
        try (Connection connection = getConnection()) {
            ...
            try (Statement statement = connection.createStatement()) {
                ...
                LifeStealZ.LOGGER.error("Failed to get eliminated players from SQL database:", e);
            }
        } catch (SQLException e) {
            LifeStealZ.LOGGER.error("Failed to get eliminated players from SQL database:", e);
        }
    ...
    public String export(String fileName) {
        ...
        try (Connection connection = getConnection()) {
            ...
            try (Statement statement = connection.createStatement()) {
                ...
                LifeStealZ.LOGGER.error("Failed to export player data to CSV file:", e);
                return null;
            }
        } catch (SQLException e) {
            LifeStealZ.LOGGER.error("Failed to export player data to CSV file:", e);
        ...
        try (BufferedReader reader = new BufferedReader(new FileReader(filePath));
            ...
            try (PreparedStatement statement = connection.prepareStatement(sql)) {
                ...
                while ((line = reader.readLine()) != null) {
                    ...
                    if (data.length != 8) {
                        LifeStealZ.LOGGER.error("Invalid CSV format. Expected 8 columns, but got {}", data.length);
                        continue;
                    }
                    ...
                    if (batchSize % 500 == 0) {
                        LifeStealZ.LOGGER.info("Imported " + totalRows + " player data entries. Committing batch...");
                ...
                LifeStealZ.LOGGER.info("Imported " + totalRows + " player data entries in " + (endTime - startTime) + "ms");

            } catch (SQLException e) {
                connection.rollback();
                LifeStealZ.LOGGER.error("Failed to import player data:", e);
            } finally {
                connection.setAutoCommit(true);
            }

        } catch (IOException | SQLException e) {
            LifeStealZ.LOGGER.error("Failed to read CSV file:", e);
        }
    }
    ...
    public int reviveAllPlayers(int minHearts, int reviveHearts, int maxRevives, boolean bypassReviveLimit) {
        ...
        try (Connection connection = getConnection()) {
            ...
            try (PreparedStatement pstmt = connection.prepareStatement(sql)) {
                ...
                LifeStealZ.LOGGER.error("Failed to revive all players in SQL database:", e);
            }
        } catch (SQLException e) {
            LifeStealZ.LOGGER.error("Failed to revive all players in SQL database:", e);
        }
    ...
    public List<String> getPlayerNames() {
        ...
        try (Connection connection = getConnection()) {
            ...
            try (Statement statement = connection.createStatement()) {
                ...
                LifeStealZ.LOGGER.error("Failed to load player names from SQL database:", e);
            }
        } catch (SQLException e) {
            LifeStealZ.LOGGER.error("Failed to load player names from SQL database:", e);
        }
    ...
    public void clearDatabase() {
        try (Connection connection = getConnection()) {
            ...
            try (Statement statement = connection.createStatement()) {
                ...
                LifeStealZ.LOGGER.error("Failed to clear SQL database:", e);
            }
        } catch (SQLException e) {
            LifeStealZ.LOGGER.error("Failed to clear SQL database:", e);
        }
